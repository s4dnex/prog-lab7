  ...
  public void enable() {
    while (true) {
      ...
      try {
        ...
        Logback.getLogger().error("Failed to receive requests from users: " + e.getMessage());
        continue;
      } catch (ClassNotFoundException e) {
        Logback.getLogger().error("Failed to deserialize requests: " + e.getMessage());
        continue;
      }
      ...
      for (SocketAddress user : requests.keySet()) {
        ...
        try {
          ...
          Logback.getLogger().error("Failed to send response to " + user + ": " + e.getMessage());
        }
      }
    }
  }
